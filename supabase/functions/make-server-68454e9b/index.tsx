...
// ── Sync branding paths from Storage → KV ────────────────────────────────────
// Útil quando os arquivos foram upados manualmente no Storage sem passar pelo admin.
app.post("/make-server-68454e9b/admin/branding/sync-from-storage", adminAuth, async (c) => {
  try {
    const storage = sb().storage.from(BUCKET);

    // Lista as pastas relevantes do branding em paralelo
    const [rootRes, bgRes, ctaRes, scannerRes, footerRes] = await Promise.all([
      storage.list("branding", { limit: 200 }),
      storage.list("branding/bg", { limit: 200 }),
      storage.list("branding/cta-bg", { limit: 200 }),
      storage.list("branding/scanner", { limit: 200 }),
      storage.list("branding/footer", { limit: 200 }),
    ]);

    const rootFiles = (rootRes.data ?? []).filter((f: any) => f.metadata?.mimetype);
    const bgFiles = (bgRes.data ?? []).filter((f: any) => f.metadata?.mimetype);
    const ctaFiles = (ctaRes.data ?? []).filter((f: any) => f.metadata?.mimetype);
    const scannerFiles = (scannerRes.data ?? []).filter((f: any) => f.metadata?.mimetype);
    const footerFiles = (footerRes.data ?? []).filter((f: any) => f.metadata?.mimetype);

    // Pega o arquivo mais recente de cada tipo da pasta raiz
    const logoFile = [...rootFiles]
      .filter((f: any) => f.name.startsWith("logo"))
      .sort((a: any, b: any) => b.name.localeCompare(a.name))[0];
    const faviconFile = [...rootFiles]
      .filter((f: any) => f.name.startsWith("favicon"))
      .sort((a: any, b: any) => b.name.localeCompare(a.name))[0];

    // Constrói os paths completos
    const logoPath = logoFile ? `branding/${logoFile.name}` : undefined;
    const faviconPath = faviconFile ? `branding/${faviconFile.name}` : undefined;
    const ctaBgPath = ctaFiles.length > 0
      ? `branding/cta-bg/${[...ctaFiles].sort((a: any, b: any) => b.name.localeCompare(a.name))[0].name}` : undefined;
    const scannerImagePath = scannerFiles.length > 0
      ? `branding/scanner/${[...scannerFiles].sort((a: any, b: any) => b.name.localeCompare(a.name))[0].name}` : undefined;
    const footerImagePath = footerFiles.length > 0
      ? `branding/footer/${[...footerFiles].sort((a: any, b: any) => b.name.localeCompare(a.name))[0].name}` : undefined;
    const backgroundPaths = [...bgFiles]
      .sort((a: any, b: any) => a.name.localeCompare(b.name))
      .map((f: any) => `branding/bg/${f.name}`);
    ...
    const updated: any = {
      ...
      ...(ctaBgPath !== undefined ? { ctaBgPath } : {}),
      ...(scannerImagePath !== undefined ? { scannerImagePath } : {}),
      ...(footerImagePath !== undefined ? { footerImagePath } : {}),
      ...(backgroundPaths.length > 0 ? { backgroundPaths } : {}),
    };

    await kv.set(`${KV}branding`, updated);
    bustBrandingCache();

    console.log(`[branding-sync] logo=${logoPath}, favicon=${faviconPath}, bg=${backgroundPaths.length}, cta=${ctaBgPath}`);
    ...
    return c.json({
      ...
      found: {
        ...
        ctaBg: !!ctaBgPath, scannerImage: !!scannerImagePath, footerImage: !!footerImagePath,
      },
    });
  } catch (err) {
    return c.json({ error: `Erro ao sincronizar branding: ${err}` }, 500);
  }
});
...
app.get("/make-server-68454e9b/admin/events", adminAuth, async (c) => {
  try {
    ...
    const raw = (await Promise.all(ids.map((id) => kv.get(`${KV}event:${id}`)))).filter(Boolean) as any[];
    raw.sort((a: any, b: any) => a.date.localeCompare(b.date));
    const events = await Promise.all(raw.map(withEventCoverUrl));
...
// Helper: attach a signed coverUrl to an event object if it has a coverPath
async function withEventCoverUrl(event: any): Promise<any> {
  if (!event?.coverPath) return event;
  const { data } = await sb().storage.from(BUCKET).createSignedUrl(event.coverPath, 3600);
  return { ...event, coverUrl: data?.signedUrl ?? null };
}
...
app.get("/make-server-68454e9b/events/:id", async (c) => {
  try {
    ...
    return c.json({ event: await withEventCoverUrl(event) });
  } catch (err) {
...
// Upload event cover image (admin)
app.post("/make-server-68454e9b/admin/events/:id/cover", adminAuth, async (c) => {
  try {
    const id = c.req.param("id");
    const existing: any = await kv.get(`${KV}event:${id}`);
    if (!existing) return c.json({ error: "Evento não encontrado" }, 404);
    ...
    const storagePath = `events/covers/${id}-${Date.now()}.${ext}`;
    ...
    if (existing.coverPath) {
      await sb().storage.from(BUCKET).remove([existing.coverPath]).catch(() => {});
    }

    const updated = { ...existing, coverPath: storagePath, updatedAt: new Date().toISOString() };
    await kv.set(`${KV}event:${id}`, updated);

    const { data: signData } = await sb().storage.from(BUCKET).createSignedUrl(storagePath, 3600);
    return c.json({ coverUrl: signData?.signedUrl ?? null, coverPath: storagePath });
  } catch (err) {
    console.log("Erro ao fazer upload do cover do evento:", err);
    return c.json({ error: `Erro ao fazer upload: ${err}` }, 500);
  }
});

// Delete event cover image (admin)
app.delete("/make-server-68454e9b/admin/events/:id/cover", adminAuth, async (c) => {
  try {
    const id = c.req.param("id");
    const existing: any = await kv.get(`${KV}event:${id}`);
    if (!existing) return c.json({ error: "Evento não encontrado" }, 404);
    ...
    const updated = { ...existing, coverPath: null, coverUrl: null, updatedAt: new Date().toISOString() };
    await kv.set(`${KV}event:${id}`, updated);
    return c.json({ success: true });
  } catch (err) {
    console.log("Erro ao remover cover do evento:", err);
    return c.json({ error: `Erro ao remover cover: ${err}` }, 500);
  }
});
...
// ── Gemini Face Verification (pública — chave fica server-side) ────────────────────────────────────────

app.post("/make-server-68454e9b/ai/verify-faces", async (c) => {
  const apiKey = Deno.env.get("ANTHROPIC_API_KEY");
  if (!apiKey) {
    // Serviço opcional: degrada graciosamente se a chave não estiver configurada
    return c.json({ unavailable: true, results: [] }, 503);
  }

  try {
    const { selfieBase64, selfieMimeType, photos } = await c.req.json();

    if (!selfieBase64 || !Array.isArray(photos) || photos.length === 0) {
      return c.json({ error: "selfieBase64 e photos[] são obrigatórios" }, 400);
    }
    if (photos.length > 20) {
      return c.json({ error: "Máximo de 20 fotos por requisição" }, 400);
    }

    const results = await claude.verifyFaceMatches(
      selfieBase64,
    ...
    const confirmed = results.filter((r) => r.verified).length;
    console.log(`[Claude] Verificou ${photos.length} fotos → ${confirmed} confirmadas`);
    return c.json({ results });
  } catch (err: any) {
    console.error("[Claude] Erro na verificação:", err);
    return c.json({ error: err?.message ?? "Erro na verificação", results: [] }, 500);
  }
});
