  ...
  style.textContent = `
    @media print {
      ...
      .ef-protected-container::after {
        content: 'Conteúdo protegido — Smart Match · Edu Santana Produções';
...
// 5 row variants — cycle through the tiled grid
const ROW_DEFS = [
  { text: 'SMART MATCH',             size: '0.95rem', weight: 900, color: 'rgba(255,255,255,0.40)', shadow: '0 1px 6px rgba(0,0,0,0.7)' },
  { text: 'EDU SANTANA PRODUÇÕES',   size: '0.62rem', weight: 700, color: 'rgba(0,255,127,0.38)',  shadow: '0 1px 5px rgba(0,0,0,0.65)' },
  { text: '◆ FOTO PROTEGIDA ◆',     size: '0.58rem', weight: 700, color: 'rgba(255,255,255,0.22)', shadow: '0 1px 4px rgba(0,0,0,0.6)' },
  { text: '© TOUR PALMEIRAS',        size: '0.62rem', weight: 700, color: 'rgba(0,212,255,0.28)',  shadow: '0 1px 5px rgba(0,0,0,0.65)' },
  { text: 'SMART MATCH',             size: '0.95rem', weight: 900, color: 'rgba(0,255,127,0.22)',  shadow: '0 1px 6px rgba(0,0,0,0.7)' },
];

// Large enough to guarantee full coverage at -28° rotation regardless of container size:
// The rotated diagonal must exceed the container's diagonal.
// We expand 150% in each direction and use 50 rows × 24 cols.
const ROWS = 50;
const COLS = 24;
        ...
        >
          ...
          >
            {Array.from({ length: ROWS }).map((_, row) => {
              const def = ROW_DEFS[row % ROW_DEFS.length];
              ...
              return (
                ...
                >
                  {Array.from({ length: COLS }).map((_, col) => (
                    ...
                    >
                      {def.text === 'SMART MATCH' ? watermarkText : def.text}
                    </span>
                  ))}
                </div>
              );
